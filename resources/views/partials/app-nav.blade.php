...
<nav class="app-nav" aria-label="Main">
    <div class="dropdown-container">
        @foreach ($items as $item)
            <div class="dropdown">
                <button class="btn dropdown-toggle nav-drop-btn" type="button" data-bs-toggle="dropdown"
                    aria-expanded="false">
                    {{ $item['label'] }}
                </button>
                <ul class="dropdown-menu">
                    @foreach ($item['subheadings'] as $shortcut)
                        <li><a class="dropdown-item app-nav__link"
                                href="{{ route($shortcut['route']) }}">{{ $shortcut['label'] }}</a></li>
                    @endforeach
                </ul>
            </div>
            </a>
